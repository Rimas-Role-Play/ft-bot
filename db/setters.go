...
func DeleteDiscordUser(pid string) {
	rows, err := db.Query("delete from discord_users where discord_uid = ?", pid)
	defer rows.Close()
	if err != nil {
...
func InsertMessageLog(channelId string, messageId string, author *discordgo.User, mType discordgo.MessageType) {
	rows, err := db.Query("INSERT INTO discord_messages_logs (channel_id, message_id, author_id, author_name, author_discriminator, message_type, time) VALUES (?,?,?,?,?,?,NOW())", channelId, messageId, author.ID, author.Username, author.Discriminator, mType)
	defer rows.Close()
	if err != nil {
...
func InsertVehicle(pid string, classname string) {
	rows, err := db.Query("INSERT INTO vehicles SET servermap = 'RRpMap',classname = ?, pid = ?, plate = ?,"+
